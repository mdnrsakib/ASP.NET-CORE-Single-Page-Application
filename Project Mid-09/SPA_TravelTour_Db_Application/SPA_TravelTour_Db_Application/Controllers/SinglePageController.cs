    ...
    public class SinglePageController : Controller
    {
        ...
        public async Task<IActionResult> EditTravelAgent(int id)
        {
            ...
            var data = await db.TravelAgents
                ...
                .FirstOrDefaultAsync(x => x.TravelAgent == id);
        ...
        public async Task<IActionResult> EditTravelAgent(TravelAgent t)
        {
            if (ModelState.IsValid)
            {
                await db.TravelAgents.AddAsync(t);
                await db.SaveChangesAsync();
                return Json(t);
            }
            //if (ModelState.IsValid)
            //{
            //    var existing = await db.Orders.Include(x => x.Customer).FirstAsync(x => x.OrderID == o.OrderID);
            //    existing.OrderDate = o.OrderDate;
            //    existing.DeliveryDate = o.DeliveryDate;
            //    existing.CustomerId = o.CustomerId;
